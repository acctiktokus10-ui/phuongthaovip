...
```
UPLOAD_PASSWORD     = phuongthaovip9999
UPSTASH_REDIS_REST_URL    = https://xxx.upstash.io
UPSTASH_REDIS_REST_TOKEN  = xxxxxxxxxx
```

4. Deploy → Lấy URL dạng `https://phuongthaovip.vercel.app`
...
## 🔗 Đồng bộ sang hoan-vi-web (web hiển thị cho khách)

Mỗi khi có upload donhang/vitien mới (`/api/upload`), hoặc bot ghi da_nhan sau khi
rút tiền (`POST /api/data/danhan`), dữ liệu sẽ được gửi kèm sang `hoan-vi-web-main`
để web đó hiển thị đơn hàng & ví tiền cho khách theo My ID.

Cần thêm 2 biến môi trường trên Vercel của **phuongthaovip-main**:

```
HOANVI_SYNC_URL = https://<domain-hoan-vi-web-cua-ban>.vercel.app/api/sync-data
SYNC_SECRET     = <chuỗi bí mật tự đặt>
```

`SYNC_SECRET` phải khớp với biến `SYNC_SECRET` đặt trên Vercel của **hoan-vi-web-main**,
và khớp với hằng `SYNC_SECRET` trong `bot_data_loader.py` (để bot cũng đẩy được da_nhan
thẳng sang hoan-vi-web mà không cần qua đây).

Nếu chưa deploy hoan-vi-web hoặc chưa cấu hình `HOANVI_SYNC_URL`, việc upload ở đây
vẫn hoạt động bình thường — chỉ bỏ qua bước đồng bộ (ghi log cảnh báo).
